...
struct LidarEdgeFactor {
    ...
    LidarEdgeFactor(Eigen::Vector3d curr_point_,
        ...
        s = s_; //s = lidar_const = 20
    }

    template <typename T> bool operator()(const T *q, const T *t, T *residual) const { //4   3   1
        Eigen::Matrix<T, 3, 1> cp{T(curr_point.x()), T(curr_point.y()), T(curr_point.z())};
        Eigen::Matrix<T, 3, 1> lpa{T(last_point_a.x()), T(last_point_a.y()), T(last_point_a.z())};
        Eigen::Matrix<T, 3, 1> lpb{T(last_point_b.x()), T(last_point_b.y()), T(last_point_b.z())};

        Eigen::Quaternion<T> q_last_curr{q[0], q[1], q[2], q[3]};//tempTrans[] 待优化的位资矩阵

        Eigen::Matrix<T, 3, 1> t_last_curr{t[0], t[1], t[2]};

        Eigen::Matrix<T, 3, 1> lp;
        lp = q_last_curr * cp + t_last_curr; //投影到世界坐标系

        Eigen::Matrix<T, 3, 1> nu = (lp - lpa).cross(lp - lpb); //note: lpa 和 lpb是以质
    ...
    static ceres::CostFunction *Create(const Eigen::Vector3d curr_point_,
                                       ...
                                       double s_) { //residual 1   param: 3  param: 4   自动求导
        return (new ceres::AutoDiffCostFunction<LidarEdgeFactor, 1, 4, 3>(new LidarEdgeFactor(curr_point_, last_point_a_, last_point_b_,  s_)));
    }
...
struct LidarPlaneNormIncreFactor {
    LidarPlaneNormIncreFactor(Eigen::Vector3d curr_point_,
                              ...
                              double negative_OA_dot_norm_): curr_point(curr_point_),
    ...
    template <typename T> bool operator()(const T *q, const T *t, T *residual) const {
        ...
        residual[0] = norm.dot(point_w) + T(negative_OA_dot_norm); //感觉这个评定标准不是太合理
        return true;
    }
